// Package registry provides an etcd version 3 registry
package registry
...
func jsonEncode(s *Service) string {
	b, _ := json.Marshal(s)
	return string(b)
}

func jsonDecode(ds []byte) *Service {
	var s *Service
	json.Unmarshal(ds, &s)
	return s
}
...
func (e *etcdv3Registry) Register(s *Service, opts ...RegisterOption) error {
	...
	for _, node := range s.Nodes {
		...
		if lgr != nil {
			_, err = e.client.Put(ctx, nodePath(service.Name, node.Id), jsonEncode(service), clientv3.WithLease(lgr.ID))
		} else {
			_, err = e.client.Put(ctx, nodePath(service.Name, node.Id), jsonEncode(service))
		}
		if err != nil {
			return err
		}
...
func (e *etcdv3Registry) GetService(name string) ([]*Service, error) {
	...
	for _, n := range rsp.Kvs {
		if sn := jsonDecode(n.Value); sn != nil {
